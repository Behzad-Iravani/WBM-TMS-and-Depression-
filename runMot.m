% -*- coding: 'UTF-8' -*-
function runMot(Empirical, varagrin)

if exist("varagrin","var")
    if length(varagrin)>1
...
Mot.randindex   = rand(Mot.number_iter,numel(Empirical.TC))>.5;
%% ----------------------------------- MonteCarlo -----------------------------------------------------------
...
for mot_ = 1:Mot.number_iter
    ...
    corr_staticFC = nan(49140,1);%nan(58575,1);
    MS_s = nan(49140,1);
    Sync_s = nan(49140,1);

    for nS = 1:49140 % total number of simulation
