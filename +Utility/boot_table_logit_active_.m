function [T_boot, CoeffName, idx] = boot_table_logit_active_(T_select,model)
...
%     length(index_select.active_DEP2)]), nboot);
rng(3)
ind_boot(:,:,1) = randi(length(index_select.active_DEP1),bc*min([...
    length(index_select.active_DEP1) ...
    length(index_select.active_DEP2)]), nboot);
rng(4)
ind_boot(:,:,2) = randi(length(index_select.active_DEP2),bc*min([...
    length(index_select.active_DEP1) ...
    length(index_select.active_DEP2)]), nboot);

idx = permute(ind_boot,[1,3,2]);
idx = cat(2,...
    index_select.active_DEP1(idx(:,1,:)),...
    index_select.active_DEP2(idx(:,2,:)));
clear myfitlm
...
try
    ...
    options.MaxIter = 1e5;
    options.TolX    = 1e-6;
    options.RobustWgtFun = 'logistic';
    beta             = stepwiseglm(T_select,model,...
